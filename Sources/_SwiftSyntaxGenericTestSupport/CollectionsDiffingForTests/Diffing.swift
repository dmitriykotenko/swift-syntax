...
extension RangeReplaceableCollection {
  ...
  public func applying(_ difference: CollectionDifferenceForTests<Element>) -> Self? {
...
extension BidirectionalCollection {
  ...
  public func differenceForTests<C: BidirectionalCollection>(
    from other: C,
    by areEquivalent: (C.Element, Element) -> Bool
  ) -> CollectionDifferenceForTests<Element>
...
extension BidirectionalCollection where Element: Equatable {
  ...
  public func differenceForTests<C: BidirectionalCollection>(
    from other: C
  ) -> CollectionDifferenceForTests<Element> where C.Element == Self.Element {
    return differenceForTests(from: other, by: ==)
  }
}
...
) -> CollectionDifferenceForTests<C.Element>
where
...
C.Element == D.Element
{
  ...
  func _formChanges(
    ...
    using trace: [_V]
  ) -> [CollectionDifferenceForTests<C.Element>.Change] {
    var changes = [CollectionDifferenceForTests<C.Element>.Change]()
  ...
  return _withContiguousStorage(for: old) { a in
    return _withContiguousStorage(for: new) { b in
      return CollectionDifferenceForTests(_formChanges(from: a, to: b, using:_descent(from: a, to: b)))!
    }
  }
}
